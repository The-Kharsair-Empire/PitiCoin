...
def order():
    ...
    db.insert_entry('CustomerOrder', date_time="NOW()", Orderred_by=uid, status='ordered')
    db.commit()
    last_id = db.run_query('SELECT LAST_INSERT_ID();')[0][0] #last inserted "order" id
    ...
    for e in purchased_items:
        ...
        for i in range(amount):
            db.insert_entry('OrderLineItem', Order_ID=int(last_id), Item_ID=int(item_id))
    ...
    if use_token:
        call_data = coins["JDC"].getTrasactionData('sendTokenFromClient', coins["JDC"].w3.toWei(total_price, "ether"))
        print(call_data)
        ...
        db.insert_entry("Transaction", Order_ID=int(last_id), date_time="NOW()", Purchase_with='AUD', value=int(total_price_aud))
    ...
    # item_info = db.get_item(selected_item)
    # reward = item_info[4] * amount
    # price_aud = item_info[2]
    # price_jdc = item_info[3]

    # print(item_info[0])
    # print(client_wallet_address)
    # print(reward)
    # print("use token: ", use_token)

    # before = coins["JDC"].getBalance(client_wallet_address)
    # print(before)

    # if use_token:
    #     print("oh yeah!")
    #     if price_jdc * amount > before:
    #         print("oh no!")
    #         return jsonify({"status": -1, "message": "no enough token, buy with your money first"})
    #     else:

    #         coins["JDC"].sendTokenFromClient(client_wallet_address, reward)
    #         return jsonify({"status": 0, "message": f"success, you just use {price_jdc * amount} JDC"})

    # else:
    #     coins["JDC"].sendTokenToClient(client_wallet_address, reward)
    #     return jsonify({"status": 0, "message": f"success, you just use {price_aud * amount} AUD"})
...
@app.route("/to_shopping/<uname>/<wallet>", methods=["GET"])
def to_shopping(uname, wallet):
